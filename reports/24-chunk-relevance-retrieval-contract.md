...
- preserve evidence provenance for answer and memo generation; and
...
retrieval tools. This is an authors' interpretation of the historical context, not documented
...
baseline. Simple Mode and Deep Research are product/research directions. The consumer argument
therefore combines current direct and indirect dependencies with actively developed and proposed
uses; it does not present every use as deployed.
...
DCG and NDCG combine system result positions with externally supplied relevance judgments. The
tool's `_score`, similarity, or MMR score is not ground-truth relevance, and retaining rank alone
does not make an evaluation valid. However, stable chunk identity, actual returned rank, and the
applied retrieval pipeline make evaluations reproducible and attributable
...
- **Proposed design:** the authors' recommendation, not an agreed owner-team decision.
...
- **Fusion:** combination of outputs or scores from multiple retrievers, such as RRF or weighted
  score addition.
- **Reranking:** reordering an existing candidate set using an additional algorithm such as MMR.
...
No checked code proves that index-time and query-time models must match. A mismatch would make the
vector comparison invalid or materially degrade it. This is a reasoned risk, not an observed
production incident.
...
The linked Slack thread is the dated, author-verified source for the production values as checked
when the message was posted. It reported text-search-only indexing together with
`Bm25SearchWithMmr`. This report does not re-verify those values or claim they are current now
([Slack discussion](https://kcura-pd.slack.com/archives/C05PKQYTZTM/p1788349409928209)).
...
**Status: current V2/V3 implementation, planned MCP V4 contract, and authors' assessment are
...
- ungrouping does not recover the original global order unless each chunk retained rank; and
- grouping can remain a consumer helper without determining the shared retrieval contract.

The proposed contract therefore uses a flat ranked chunk list. This is an authors' recommendation,
not an agreed change to qna-service or planned MCP V4.
...
**Status: authors' recommendation for discussion. It is not an agreed qna-service,
...
- whether non-relevance field sorting is supported; and
...
- optional component scores/ranks for fusion diagnostics; and
...
- the service uses a stable tie-break, for example `documentId ASC, chunkId ASC`; and
...
**Status: authors' recommendations requiring owner review and agreement.**
